
    
    ==========================
    https://data-flair.training/blogs/python-chatbot-project/
    
    https://www.kaggle.com/python10pm/matplotlib-learn-more-than-60-plots-step-by-step
    
    https://www.youtube.com/channel/UCYO_jab_esuFRV4b17AJtAw/playlists
    
    https://courses.analyticsvidhya.com/courses/get-started-with-scikit-learn-sklearn
    
    https://www.youtube.com/watch?v=JL_grPUnXzY&list=PLeo1K3hjS3us_ELKYSj_Fth2tIEkdKXvV  -  LEARNING RESORUCES FOLDER CONTRIBUTOR
    
    https://www.youtube.com/watch?v=XcLO4f1i4Yo
    
    
    
    
    +++++++++++
    matplotlib:
    https://www.kaggle.com/saurav9786/matplot-tutorial-for-everyone
    https://github.com/rougier/matplotlib-tutorial
